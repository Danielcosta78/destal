...
(function() {
        ...
        features: {
            themes: {
                ...
                apply: function() {
                    ...
                    $('head').append('<style id="premium-theme-styles">.theme-matrix{--bg-color:#000;--text-color:#0f0;--card-bg:#121212;--border-color:#0f0}.theme-deep-blue{--bg-color:#001a33;--text-color:#cce6ff;--card-bg:#003366;--border-color:#4da6ff}.theme-dark-red{--bg-color:#1a0000;--text-color:#ff9999;--card-bg:#330000;--border-color:#ff4d4d}.theme-nature-green{--bg-color:#001a00;--text-color:#99ff99;--card-bg:#003300;--border-color:#4dff4d}.theme-sunset-glow{--bg-color:#fff5e6;--text-color:#b35c00;--card-bg:#ffdca6;--border-color:#f2994a}.theme-ocean-breeze{--bg-color:#e0f7fa;--text-color:#006064;--card-bg:#b2ebf2;--border-color:#00acc1}.theme-forest-mist{--bg-color:#e8f5e9;--text-color:#2e7d32;--card-bg:#c8e6c9;--border-color:#66bb6a}.theme-lavender-dream{--bg-color:#f3e5f5;--text-color:#6a1b9a;--card-bg:#e1bee7;--border-color:#ab47bc}</style>');
            ...
            emojis: {
                ...
                apply: function() {
                    if ($('.premium-emojis-toggle').length === 0) {
                        $('#sidebar .sidebar-content').prepend('<div class="sidebar-section premium-emojis-toggle" style="margin-bottom:10px;"><button class="btn btn-sm btn-secondary btn-block" style="white-space: nowrap;">Show Premium Emojis</button></div>');
                        $('#sidebar .sidebar-content').prepend('<div class="sidebar-section premium-emojis-container" style="display:none;"><h5 class="sidebar-title"><i class="fas fa-smile"></i> Premium Emojis</h5><div class="emoji-grid" style="display:grid;grid-template-columns:repeat(3,1fr);gap:18px;padding:10px;font-size:28px;cursor:pointer;user-select:none;justify-items:center;align-items:center;"><span>🦄</span><span>🧙‍♂️</span><span>🛸</span><span>🐉</span><span>🍀</span><span>🦜</span><span>🎭</span><span>🌌</span><span>🪐</span><span>🦥</span><span>🦦</span><span>🦩</span><span>🧛‍♀️</span><span>🧟‍♂️</span><span>🧞‍♂️</span><span>🧜‍♀️</span><span>🦚</span><span>🪶</span><span>🪄</span><span>🧩</span><span>🛡️</span><span>🏺</span><span>🦋</span><span>🐾</span><span>🧿</span><span>🕯️</span><span>⚗️</span><span>🧪</span><span>🧸</span></div></div>');
    ...
    function hideActivationInput() {
        $('.premium-key-input, .activate-btn, .paypal-container').hide();
        $('.remove-premium-btn').show();
    }

    function showActivationInput() {
        $('.premium-key-input, .activate-btn, .paypal-container').show();
        $('.remove-premium-btn').hide();
    }

    function addPayPalSDK() {
        if (!$('script[src^="https://www.paypal.com/sdk/js"]').length) {
            const script = document.createElement('script');
            script.src = 'https://www.paypal.com/sdk/js?client-id=BAAkPzm10hAQfwuLQe5bVE_a0ncc5lBvxDLnhJKcBHgSHts8gGaPMjJaOA6Nc4Z34j2IMORrfkjgEnvBB8&components=hosted-buttons&disable-funding=venmo&currency=USD';
    ...
    function initPayPalButton() {
        if (typeof paypal !== 'undefined' && !activation.active) {
    ...
    function addPremiumSection() {
        ...
        const premiumSection = `
            <div class="sidebar-section premium-section">
                ...
                ${!activation.active ? '<div id="paypal-container-V7HLZPTQNAF8W" class="paypal-container mt-3"></div>' : ''}
            </div>

            <div class="sidebar-section premium-themes-container" style="display: ${activation.active ? 'block' : 'none'}; margin-top: 15px;">
        ...
        $('#sidebar .sidebar-content').prepend(premiumSection);
        ...
        $('.activate-btn').on('click', () => {
            ...
            if (res.success) {
                ...
                hideActivationInput();
                $('.premium-section .form-group').append('<button class="btn btn-danger btn-sm btn-block mt-2 remove-premium-btn">Remove Premium</button>');
                $('.premium-themes-container').show();
